...
# Process images of this size. Note that this differs from the original TRANCOS
# image size of 480 x 640. If one alters this number, then the entire model
...
def read_trancos(filename_queue):
...
def gen_random_pos(imSize, pw, N):
  ...
  y=np.random.randint(dy,ih-dy,N).reshape(N,1)
  x=np.random.randint(dx,iw-dx,N).reshape(N,1)

  pos = np.hstack(((y-dy)/float(int(ih)), (x-dx)/float(int(iw)), (y+dy+1)/float(int(ih)), (x+dx+1)/float(int(iw))))
...
def distorted_inputs(data_dir, trainval_list, batch_size):
  ...
  Args:
    data_dir: Path to the TRANCOS data directory.
    trainval_list: The list of TRANCOS training / evaluation files.
    batch_size: Number of images per batch.
